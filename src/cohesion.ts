...
export default class Cohesion {
  ...
  public onTitleUpdateCallbacks: Array<(title: string, explicitSet: boolean) => void> = [];
  ...
  private openTab(url: string) {
    ...
    newContent.webContents.on('did-finish-load', () => {
      console.log('[cohesion] did-finish-load fired for:', url);
      try {
        const scriptPath = path.join(__dirname, 'js', 'test.js');
        console.log('[cohesion] reading script from:', scriptPath);
        const script = readFileSync(scriptPath, 'utf-8');
        console.log('[cohesion] script loaded, injecting...');
        newContent.webContents.executeJavaScript(script).then(() => {
          console.log('[cohesion] script injected successfully');
        }).catch((err: Error) => {
          console.error('[cohesion] executeJavaScript failed:', err);
        });
      } catch (err) {
        console.error('[cohesion] failed to inject script:', err);
      }
    });
    ...
    newContent.webContents.on('page-title-updated', (_event, title, explicitSet) => {
      ...
      if (explicitSet && newContent === this.currentTab) {
        ...
        this.onTitleUpdateCallbacks.forEach(cb => cb(cleaned, explicitSet));
      }
  ...
  constructor() {
    ...
    this.moduleManager = new ModuleManager([
      ...
      whatsnewModule,
      notificationModule,
      new MenuModule(this, this.window, spellcheckModule, whatsnewModule, notificationModule),
